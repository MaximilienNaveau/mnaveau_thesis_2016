...
for i in range(n):
  figure.append(plt.figure(i,figsize=(15,7.5)))
  graph.append(figure[i].add_subplot(111))
  plt.ylabel("meter (m)")
  plt.xlabel("time (s)")
  if i==0:
    minigraph = plt.axes([0.38, 0.16, .3, .2])
...
graph[0].plot(x[0] , dataset[0][1:,[13]], "b"   , label='CoP',linewidth=2)
graph[0].plot(x[1] , dataset[1][1:,[44]], "g--" , label='CoPmultibody',linewidth=3)
graph[0].plot(x[0] , dataset[0][1:,[44]], "r"   , label='CoPmultibody filtered', dashes=[8, 4, 2, 4, 2, 4],linewidth=3)#1628
#graph[0].plot(x[4] , dataset[4][1:,[44]], "m" , label='CoPmultibody filtered lower body', dashes=[8, 2, 2, 4, 2, 4],linewidth=3)
graph[0].legend(bbox_to_anchor=(0.75, 0.25), loc=2, borderaxespad=0., prop={'size':20})
plt.ylabel("meter (m)")
plt.xlabel("time (s)")
...
graph[0].xaxis.label.set_fontsize(25)
graph[0].yaxis.label.set_fontsize(25)


for item in (minigraph.get_xticklabels() + minigraph.get_yticklabels()):
  item.set_fontsize(25)
minigraph.xaxis.label.set_fontsize(20)
minigraph.yaxis.label.set_fontsize(20)
...
if savetofile :
  figure[0].savefig('/home/mnaveau/Documents/presentations/16-thesis-defense-mnaveau/figures/nmpc_walkgen/copmbpres.pdf',bbox_inches='tight',format="pdf", dpi=900)
  figure[0].savefig('./copmbpres.pdf',bbox_inches='tight',format="pdf", dpi=900)
  figure[1].savefig('./forcezpres.pdf',bbox_inches='tight',format="pdf", dpi=900)
