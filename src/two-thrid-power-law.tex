

\begin{frame}{Transferring the human power-law to humanoid robots}
\framesubtitle{
  \textcolor{green!30!black!80}
  {
    (K. Matan, BioRob 2016)
  }
}
  \begin{itemize}
    \item Observation : Human moves according to the two-third power-law\\
    \begin{center}
    \tikz[baseline]{\node[fill=txtcolor3,anchor=base]{$ v $};} =
    \tikz[baseline]{\node[fill=blue!50,anchor=base]{$ \gamma $};}
    \tikz[baseline]{\node[fill=green!50,anchor=base]{$ \kappa $};}
    \textsuperscript{\tikz[baseline]{\node[fill=red!50,anchor=base]{$-\beta$};}}
    \end{center}
\vspace*{-0.5cm}
\scalebox{1}{
    \begin{tikzpicture}
   \filldraw [gray] ([xshift=-3cm,yshift=-2cm] current page.center) circle (2pt)
      % +(3,0) circle (2pt)
      % +([xshift=2.7488cm,yshift=-1.1762cm] current page.center) circle (2pt)
                     +(6,0) circle (2pt);
    \node[color=gray] at ([xshift=-3.75cm,yshift=-2cm] current page.center) {Start};
    \node[color=gray] at ([xshift=3.75cm,yshift=-2cm] current page.center) {Goal};
    
    % Trajectory
      \draw[thick,color=red]
       ([xshift=-3cm,yshift=-2cm] current page.center) .. controls +(3,0) and +(-1,2) .. +(6,0);

      % Point in gray
      \filldraw [gray] ([xshift=1.75cm,yshift=-1.1262cm] current page.center) circle (2pt);
      % Name of the point: P
      \node at ([xshift=1.75cm,yshift=-0.75cm]current page.center) {$P$};


        % Osculting circle
        \draw[color=blue] ([xshift=1.7cm,yshift=-2.125cm]current page.center) circle (1cm);
        % Drawig Ray to the plot 
        \draw ([xshift=1.7cm,yshift=-2.125cm]current page.center) -- ([xshift=1.75cm,yshift=-1.1262cm]current page.center);
        % Ray label
        \node at ([xshift=2.0cm,yshift=-1.6262cm]current page.center) {$R$};
        % Tangent vector to the osculting circle along the trajectory (velocity vector)
        \draw ([xshift=0.7512cm,yshift=-1.0762cm]current page.center) -- ([xshift=2.7488cm,yshift=-1.1762cm]current page.center);

    % Speed
      \draw[-latex,thick,color=orange] ([xshift=1.75cm,yshift=-1.1262cm]current page.center) -- ([xshift=2.4991cm,yshift=-1.1637cm]current page.center)
      node[above] {$v$};

    \draw[-latex,thick,color=orange] ([xshift=-3cm,yshift=-2cm]current page.center) -- ([xshift=-2.5cm,yshift=-2cm]current page.center)
    node[above]{$v_{Start}$};
    
    \draw[-latex,thick,color=orange] ([xshift=3.0cm,yshift=-2cm]current page.center) -- ([xshift=3.25cm,yshift=-2.5cm]current page.center)
    node[below]{$v_{Goal}$};
  \end{tikzpicture}
}
    \item Does this two third power law help humanoid robots to walk?
  \end{itemize}
\end{frame}

\begin{frame}{Drift compensation}
\vspace*{-0.8cm}  
  \begin{center}
    \includegraphics[height = 5.0cm]{two_third/morphing.pdf}
  \end{center}
  \vspace*{-0.5cm}
  \begin{itemize}
    \item Contracting vector field towards cyclic reference trajectories.
    \item (a) unit cycle oscillator, (b) elliptic morphing
    \item (c) $\rightarrow$ (f) $\beta=\{-\frac{1}{3},0,\frac{1}{3},\frac{2}{3}\}$
  \end{itemize}
\end{frame}

\begin{frame}{Full controller}
  \begin{center}
    \scalebox{0.7}{\input{/figures/two_third/tikz/control-scheme.tex}} \\
  \end{center}
\end{frame}

\begin{frame}{Results}
  \begin{center}
    \includegraphics[trim={1.0cm 10.0cm 1.0cm 13.0cm}, clip,height = 3.0cm]
    {two_third/Fig5d_EXPshapesSymmetric.pdf}
    %trim={<left> <lower> <right> <upper>}
    \hfill
    \begin{table}[ht]
    	\centering 
	  \begin{tabular}{| c | c | c | c | c |c |} 
		  \hline 
		  Ref.~$\beta$        & $-\frac{1}{3}$  & $0$      & \textcolor{red}{$\frac{1}{3}$} & $\frac{2}{3}$   \\
		  \hline  
		  Norm     ($m$)        & $1.416$  & $0.950$  & \textcolor{red}{$0.642$} & $1.124$ \\   
		  Orientation ($deg$)   & $76.83$  & $89.60$  & \textcolor{red}{$60.45$} & $77.28$ \\ 
		  Tangential Force ($kN \; s$) & $21.93 $ & $23.54$  & \textcolor{red}{$19.80$} &$21.01$  \\ 
		  \hline 		  
	  \end{tabular}
  \end{table}
  \end{center}
  \blfootnote{(Karklinsky et al. BioRob 2016)}
%  
%  Globally, the     values of the simulated motions reflect the ones of     the reference trajectory 
%
%The position of maximal speed on simulated trajectories is slightly shifted  (delayed)
%
%Unpredictably, for         ,  an oscillatory curvature-dependent speed profile is observed
%
%The controlled motions take more time than the reference movement. The higher     the faster the motion. 
\end{frame}





