...
\begin{tikzpicture}[auto, node distance=2cm,>=latex]
    ...
    \node [input]  at (-0.2, 0.0) (input)  {};
    \node [input]  at (3, 0.0) (velocity) {};
    \node [input]  at (5.5, -1.0) (feedback)  {};
    \node [input]  at (14.7, -2.9) (feedback2)  {};
    \node []    at ( 0.0, 0.0) (sumin)  {};
    %\node [output]    at ( 15, 0.0) (sumout) {};

    \node [block] at (5.0,-1.7) (lwc) {
        Left Wrist\\
        Hybrid\\
        Controller        
    };

    \node [block] at (1.6,0.0) (walking) {
        Walking \\
        Task    
    };

    \node [block] at (5.0,0) (wpg) {
        Walking\\
        Pattern\\
        Generator
    };
...
%        Filter
%    };
    \node [block] at (8.5,0) (sot) {
        Generalized\\
        Inverse\\
        Kinematics
    };
    \node [block] at (12.5, 0) (system) {
    		Robot Hardware\\
    		{\small Simulation/Robot}\\
    		and\\
    		{\small motion capture system}
    	};
...
%    \draw [->] (dyn) -- node {\small $\tilde{c}^{\,ref},f^{ref}$} (sot);
    \draw [->] (wpg) -- node {\small ${\mathbf{p}_c}^{\,{\text {ref}}},{\mathbf{p}_f}^{\,{\text {ref}}}$} (sot);
    \draw [->] (sot) -- node {\small ${\mathbf q}^{\,{\text{ref}}},\dot{{\mathbf q}}^{\,{\text{ref}}}$} (system);
    
    \draw [->] (lwc) -| node [near start, above]{\small ${\mathbf{p}_{lw}}^{\,{\text {ref}}}$} (sot);
...
%    \draw [->] (feedback) -| node {} (wpg);
    ...
    \draw [->] (feedback2) -| node [near start, above]{\small $\mathbf{f}_{lw},\mathbf{p}_{w},\mathbf{p}_{lw},\mathbf{p}_{f}$} (lwc);
    \draw [->] (feedback2) -| node [near end, right]{\small $\mathbf{p}_{ch}$} (walking);
